...
const Fazer = ({ deleteTarefa }) => {
  ...
  //TODAS AS TAREFAS "FAZER"
  const [datas, setDatas] = useState([]);
  const fetch = async () => {
    await axios
      .get("https://tarefass-api.herokuapp.com/api/v1/fazerTarefas")
      .then(({ data }) => {
        setDatas(data);
      })
    ...
    <div className="column is-4">
      <div className="card has-background-grey-lighter">
        <div className="card-content">
          <p className="is-size-3 has-text-weight-semibold mb-5">Fazer ✏️</p>
          {datas.length ? (
            datas.map((data) => {
              return (
                <div
